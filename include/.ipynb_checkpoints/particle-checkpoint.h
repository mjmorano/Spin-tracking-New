...
public:
	...
	{	
		...
		S = y0;
		outputArray = storage;
		pos.x = get_uniform_prn(process_data, thread_data, ++icount, &iprn)*L.x-L.x/2.0;
		pos.y = get_uniform_prn(process_data, thread_data, ++icount, &iprn)*L.y-L.y/2.0;
		pos.z = get_uniform_prn(process_data, thread_data, ++icount, &iprn)*L.z-L.z/2.0;
		// printf("%f\n", pos.x);
		// printf("%f\n", pos.y);
